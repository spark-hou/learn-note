...
const dataTransform = (data: any) => {
  const changeData = (d: any, level = 0, index = 0, color: any = '') => {
    const data = {
      ...d,
    };
    switch (level) {
    ...
    if (d.children) {
      data.children = d.children.map((child: any, index2: number) => changeData(child, level + 1, index2, data.color || ''));
    }
    return data;
  };
  return changeData(data);
};
export {G6, dataTransform}
