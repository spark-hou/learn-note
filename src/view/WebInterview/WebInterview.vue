<template>
  <div class="WebInterview">
    <div class="g-container">
      ...
      <div class="g-block"></div>
      <a-typography-title :level="2">html</a-typography-title>
      <a-typography-title :level="2">css</a-typography-title>
      <div>
        <a-typography-title :level="4">说一说BFC（块级格式化上下文）</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
            BFC（块级格式化上下文）是 CSS 中的一个重要概念，它是一个独立的渲染区域，具有自己的布局规则和渲染规则。
      ...
      <div>
        <a-typography-title :level="4">说一说盒模型？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
            在 CSS 样式表中，当多个选择器同时作用于同一个元素时，会出现样式冲突的问题，此时就需要确定样式的优先级。
      ...
      <a-typography-title :level="2">js</a-typography-title>
      <div>
        <a-typography-title :level="4"> 说一说JS数据类型有哪些,区别是什么？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              基础类型（放在栈）：null，undefined，string，number，bigint，symbol，boolean
              引用类型（放在堆）：object
            </pre>
          </a-typography-text>
        </a-typography-paragraph>
      ...
      <div>
        <a-typography-title :level="4">关于null和undefined</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              在 JavaScript 中，null 和 undefined 都表示一个没有值的状态，但它们有一些不同的特点和用途。
      ...
      <div>
        <a-typography-title :level="4">说一说你对闭包的理解？
        </a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              闭包（Closure）是指函数能够访问并使用在其定义作用域内的变量，即使这些变量在函数被调用时已经离开了作用域。
      ...
      <div>
        <a-typography-title :level="4">说一说promise是什么与使用方法？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              Promise 是一种异步编程的解决方案，它是 ES6 引入的一个新的对象类型。
      ...
      <div>
        <a-typography-title :level="4">实现一个promise a+</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              // 定义 Promise 构造函数
      ...
      <div>
        <a-typography-title :level="4">说一说事件循环Event loop，宏任务与微任务</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              JavaScript 运行时包括一个主线程和一个消息队列，主线程执行同步代码，遇到异步任务会将它们添加到消息队列中。
      ...
      <div>
        <a-typography-title :level="4">说一说JavaScript有几种方法判断变量的类型？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              typeof：用于检查一个变量的类型，返回一个字符串，包括 "undefined"、"number"、"string"、"boolean"、"function"、"object" 和 "symbol"。
      ...
      <div>
        <a-typography-title :level="4">说一说JS实现异步的方法？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              回调函数：通过将回调函数作为参数传递给异步函数，当异步函数执行完成后调用回调函数。
      ...
      <div>
        <a-typography-title :level="4">说一说数组去重都有哪些方法？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              在 JavaScript 中，有多种方法可以对数组进行去重，常见的包括以下几种：
      ...
      <div>
        <a-typography-title :level="4">说一说call apply bind的作用和区别？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              call、apply 和 bind 都是 JavaScript 中用于绑定函数执行上下文的方法。
      ...
      <div>
        <a-typography-title :level="4">说一说this指向（普通函数、箭头函数）？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              在 JavaScript 中，this 关键字指向当前执行上下文中的对象，具体的指向方式取决于函数的调用方式和函数本身的定义方式。
      ...
      <div>
        <a-typography-title :level="4">说一说JS变量提升？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              具体来说，JavaScript 中的变量提升分为两种：函数提升和变量提升。
      ...
      <div>
        <a-typography-title :level="4">说一说map 和 forEach 的区别？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              map 和 forEach 都是 JavaScript 中常用的数组迭代方法，它们的主要区别在于返回值和对原数组的影响。
      ...
      <a-typography-title :level="2">vue</a-typography-title>
      <div>
        <a-typography-title :level="4">说一说Vue2.0 双向绑定的原理与缺陷？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              Vue2.0 中的双向数据绑定是通过数据劫持结合发布订阅模式实现的。
      ...
      <div>
        <a-typography-title :level="4">说一说Vue3.0 实现数据双向绑定的方法 ？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              Vue3.0 采用了 Proxy 对象来实现数据双向绑定，与 Vue2.0 的双向绑定方式（Object.defineProperty）相比，有以下优点：
      ...
      <div>
        <a-typography-title :level="4">说一下Diff算法？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              Diff算法是前端框架（如React、Vue等）用于高效更新视图的核心算法之一。
      ...
      <div>
        <a-typography-title :level="4">说一说 vue 的 keep-alive ？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              在 Vue 中，使用 keep-alive 组件可以将被包含的组件缓存起来，不会被销毁，以便于下一次需要时直接从缓存中读取，从而提高应用性能。
      ...
      <a-typography-title :level="2">浏览器</a-typography-title>
      <div>
        <a-typography-title :level="4">说一说cookie sessionStorage localStorage 区别？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              cookie：小 4kb 发请求自动带cookie
      ...
      <div>
        <a-typography-title :level="4">说一说跨域是什么？如何解决跨域问题？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
      ...
      <div>
        <a-typography-title :level="4">说一下浏览器输入URL发生了什么？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              URL解析：浏览器首先解析您输入的URL。这个过程将URL分解成几个组成部分，包括协议（例如HTTP或HTTPS），主机名和路径等。
      ...
      <div>
        <a-typography-title :level="4">说一下重绘、重排区别如何避免？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              在网页渲染的过程中，浏览器会进行重排（reflow）和重绘（repaint）操作，这两个操作会消耗较多的计算资源和时间，影响页面的性能和响应速度。因此，我们需要尽可能地避免重绘和重排的发生。
      ...
      <a-typography-title :level="2">进阶</a-typography-title>
      <div>
        <a-typography-title :level="4">说一说前端性能优化手段？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              减少 HTTP 请求：减少页面中的资源数量，如脚本、样式表、图像等，可以显著减少 HTTP 请求的数量，从而加快页面加载速度。
      ...
      <div>
        <a-typography-title :level="4">说一说性能优化有哪些性能指标，如何量化？</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text>
            <pre>
              性能优化的目标是提高应用程序的性能，从用户角度来看，性能指标可以分为以下几种：
